...
#######################################################################
def batchReader(filePath, maxLen=1000, minLen=0):
    '''
    Args:
        filePath (TYPE): DESCRIPTION.
        maxLen (TYPE, optional): DESCRIPTION. Defaults to 800.
    ...
    with open(filePath, 'r') as f:
        while True:
            ...
            if sequence == '':    # test if EOF
                break     
            sequence = sequence[:maxLen]  # reduce to maxLength
            target = target[:maxLen]
            length = len(sequence)
            if length<minLen:   # reject less than minLength
                continue
            if length>maxLen:   # reject greater than maxLength
                continue
            
            if length not in seqDict.keys():
                seqDict[length] = [oneHot(sequence, vocab="ARNDCEQGHILKMFPSTWYV")]
                tarDict[length] = [oneHot(target, vocab="HEC") ]
            else:
                seqDict[length].append(oneHot(sequence, vocab="ARNDCEQGHILKMFPSTWYV"))
                tarDict[length].append(oneHot(target, vocab="HEC"))
         
    # arrange in arrays of shape (Nseqs(0),Nclasses(1),maxLen(2)) -needed for CNN
    for key in seqDict.keys():
        seqDict[key] = torch.tensor( 
                            np.array(seqDict[key]).swapaxes(1, 2),
                            dtype=torch.float32,
                            requires_grad=True )
        tarDict[key] = torch.tensor( 
                            np.array(tarDict[key]).swapaxes(1, 2),
                            dtype=torch.float32,
                            requires_grad=True )
